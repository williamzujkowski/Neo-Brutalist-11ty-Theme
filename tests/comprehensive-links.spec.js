...
const ALL_PAGES = [
  ...
  { path: '/posts/breaking-design-rules-a-guide-to-creative-rebellion/', name: 'Breaking Design Rules Post' },
  { path: '/posts/color-revolution-breaking-free-from-minimalist-palettes/', name: 'Color Revolution Post' },
  { path: '/posts/building-a-neo-brutalist-theme-with-11ty/', name: 'Building with 11ty Post' },
  { path: '/posts/building-for-the-bold-a-developer-s-guide-to-neo-brutalist-web-architecture/', name: 'Building for the Bold Post' },
  { path: '/posts/the-psychology-of-brutal-design-why-our-brains-crave-visual-chaos/', name: 'Psychology of Brutal Design Post' },
  { path: '/posts/the-future-of-web-rebellion-trends-that-refuse-to-follow-rules/', name: 'Future of Web Rebellion Post' },
  // Projects
...
test.describe('Comprehensive Link Validation Suite', () => {
  ...
  test('should validate ALL external links and social media links', async ({ page }) => {
    ...
    for (const pageData of ALL_PAGES) {
      ...
      try {
        ...
        for (const link of externalLinks) {
          ...
          const socialPlatforms = ['github.com', 'linkedin.com', 'twitter.com', 'x.com', 'instagram.com',
                                 'youtube.com', 'facebook.com', 'discord', 'medium.com', 'behance.net',
                                 'dribbble.com', 'codepen.io'];
          ...
          expect(rel, `External link ${href} should have noopener for security`).toContain('noopener');
    ...
    const socialPlatforms = [...new Set(testResults.socialLinks.map(link => {
  ...
  test('should validate blog navigation and "Back to Blog" links', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(blogPostLinks.length, 7); i++) { // Test all 7 posts
      ...
      try {
        ...
        const postContent = page.locator('article, .post-content, .blog-post-content, main').first();
  ...
  test('should validate footer links and social icons', async ({ page }) => {
    ...
    for (const pageData of ALL_PAGES.slice(0, 5)) { // Test on key pages
      await page.goto(pageData.path);
      await waitForPageLoad(page);
      ...
      if (await footer.count() > 0) {
        ...
        const socialIcons = footer.locator('a[href*="github"], a[href*="linkedin"], a[href*="twitter"], a[href*="instagram"]');
        ...
        const footerLinks = footer.locator('a:not([href*="github"]):not([href*="linkedin"]):not([href*="twitter"])');
  ...
  test.afterAll(async () => {
    ...
    if (testResults.socialLinks.length > 0) {
      ...
      const platforms = [...new Set(testResults.socialLinks.map(link => {
        try {

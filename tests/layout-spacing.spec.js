    ...
    testPages.forEach(pageInfo => {
      test.describe(`${device.name} - ${pageInfo.name} Layout`, () => {
        ...
        test('should not have horizontal scrolling', async ({ page }) => {
          ...
          const overflowElements = await page.evaluate(() => {
            ...
            elements.forEach(el => {
              ...
              if (rect.right > viewportWidth + 10) { // Allow 10px tolerance
                problematic.push({
                  tagName: el.tagName,
        ...
        test('should have proper \"Back to Blog\" button alignment', async ({ page }) => {
          ...
          if (pageInfo.path === '/blog/') {
            ...
            if (blogLinks.length > 0) {
              ...
              const backButton = page.locator('.back-to-blog, .btn-back, a[href*=\"/blog\"]').first();
        ...
        test('should have adequate touch targets', async ({ page }) => {
          ...
          const interactiveElements = await page.locator('button, a, .btn, .nav-toggle, .social-icon').all();
          ...
          for (const element of interactiveElements.slice(0, 5)) {
            ...
            if (await element.getAttribute('class') &&
        ...
        test('should have proper section spacing', async ({ page }) => {
          const sections = await page.locator('section, .hero, .about, .services, .projects, .blog, .contact').all();
        ...
        test('should handle grid layouts properly on mobile', async ({ page }) => {
          const gridContainers = await page.locator('.services-grid, .projects-grid, .blog-grid, .about-grid').all();
          ...
          for (const grid of gridContainers.slice(0, 2)) {
            ...
            if (gridStyles.display === 'grid') {
              ...
              const columnCount = (gridStyles.gridTemplateColumns.match(/auto|fr|px|%|em|rem/g) || []).length;
        ...
        test('should have proper margins and padding throughout', async ({ page }) => {
          ...
          for (const container of containers.slice(0, 2)) {
            ...
            const totalHorizontalSpacing = spacing.paddingLeft + spacing.paddingRight +
                                         Math.max(0, spacing.marginLeft) + Math.max(0, spacing.marginRight);

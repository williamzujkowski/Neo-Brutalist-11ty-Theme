    ...
    test.describe(`${viewport.name} Accessibility`, () => {
      ...
      for (const url of testPages) {
        test(`${url} - Touch Target Sizes`, async ({ page }) => {
          ...
          const interactiveElements = page.locator('a, button, input, [role="button"], [tabindex]:not([tabindex="-1"])');
        ...
        test(`${url} - Color Contrast`, async ({ page }) => {
          ...
          const textElements = page.locator('p, h1, h2, h3, h4, h5, h6, a:not(.social-link), span, li').filter({ hasText: /.+/ });
        ...
        test(`${url} - Keyboard Navigation`, async ({ page }) => {
          ...
          if (count > 0) {
            ...
            let currentFocused = await page.locator(':focus').count();

...
MOBILE_DEVICES.forEach(device => {
  test.describe(`Mobile Responsiveness - ${device.name} (${device.width}x${device.height})`, () => {
    ...
    test('Blog Post - Back to Blog Button Alignment', async ({ page }) => {
      ...
      const backButton = page.locator('a[href*="blog"], .back-to-blog, .back-button, a:has-text("Back to Blog"), a:has-text("← Blog"), a:has-text("Back")');
    ...
    test('Typography Readability and Spacing', async ({ page }) => {
      ...
      if (await paragraphs.count() > 0) {
        ...
        const lineHeight = await firstParagraph.evaluate(el => window.getComputedStyle(el).lineHeight);
    ...
    test('Social Icons Footer Positioning', async ({ page }) => {
      ...
      const socialIcons = page.locator('.social, .social-icons, .social-links, footer a[href*="twitter"], footer a[href*="github"], footer a[href*="linkedin"], footer a[href*="facebook"]');
...
test('Cross-Device Layout Consistency', async ({ page }) => {
  ...
  for (const device of MOBILE_DEVICES) {
    ...
    const headerHeight = await page.locator('header').boundingBox().then(b => b?.height || 0);
    const mainWidth = await page.locator('main').boundingBox().then(b => b?.width || 0);

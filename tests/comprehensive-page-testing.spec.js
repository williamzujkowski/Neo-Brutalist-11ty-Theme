...
test.describe('Comprehensive Page Testing', () => {
  const allPages = [
    { url: '/', title: 'Home', description: 'Homepage with hero, services, projects, blog preview' },
    { url: '/about/', title: 'About', description: 'About page with content, stats, layout' },
    { url: '/services/', title: 'Services', description: 'Services page with cards and hover effects' },
    { url: '/blog/', title: 'Blog', description: 'Blog listing with grid layout and post cards' },
    { url: '/blog/getting-started-with-11ty/', title: 'Blog Post 1', description: 'Blog post with navigation and content' },
    { url: '/blog/neo-brutalist-design-principles/', title: 'Blog Post 2', description: 'Blog post with navigation and content' },
    { url: '/blog/building-fast-static-sites/', title: 'Blog Post 3', description: 'Blog post with navigation and content' },
    { url: '/blog/markdown-and-nunjucks/', title: 'Blog Post 4', description: 'Blog post with navigation and content' },
    { url: '/blog/responsive-typography/', title: 'Blog Post 5', description: 'Blog post with navigation and content' },
    { url: '/blog/performance-optimization/', title: 'Blog Post 6', description: 'Blog post with navigation and content' },
    { url: '/blog/seo-best-practices/', title: 'Blog Post 7', description: 'Blog post with navigation and content' },
    { url: '/projects/project-alpha/', title: 'Project Alpha', description: 'Project page with layout and back navigation' },
    { url: '/projects/project-beta/', title: 'Project Beta', description: 'Project page with layout and back navigation' },
    { url: '/projects/project-gamma/', title: 'Project Gamma', description: 'Project page with layout and back navigation' },
    { url: '/projects/project-delta/', title: 'Project Delta', description: 'Project page with layout and back navigation' },
    ...
    for (const viewport of mobileViewports) {
      test(`All pages responsive on ${viewport.name}`, async ({ page }) => {
        ...
        for (const testPage of allPages) {
          ...
          expect(bodyScrollWidth, `No horizontal scroll on ${testPage.url} at ${viewport.name}`).toBeLessThanOrEqual(windowInnerWidth + 2);
          ...
          if (await bodyText.count() > 0) {
            ...
            expect(fontSize, `Font size on ${testPage.url} at ${viewport.name}`).toBeGreaterThanOrEqual(14);
          }
        }
      });
    }
        ...
        for (const testPage of allPages) {
          ...
          if (await container.count() > 0) {
            ...
            if (containerBox) {
              ...
              expect(containerBox.width, `Container width on ${testPage.url} at ${viewport.name}`).toBeLessThan(viewport.width - 40);
            }
          }
          ...
          if (await navItems.count() > 1) {
            ...
            if (firstItem && secondItem) {
              ...
              expect(verticalDiff, `Navigation horizontal layout on ${testPage.url}`).toBeLessThan(10);
            }
          }
        }
      });
      ...
      if (postCount > 0) {
        ...
        if (postCount > 1) {
          ...
          if (firstPost && secondPost) {
            ...
            expect(horizontalSpacing > 10 || verticalSpacing > 10, 'Blog posts should have grid spacing').toBeTruthy();
          }
        }
      }
    });
  ...
  test.describe('Cross-Page Navigation', () => {
    ...
    test('Project back navigation', async ({ page }) => {
      ...
      for (const project of projects) {
        ...
        const backToProjects = page.locator('.back-to-projects, .back, [href="/projects/"], [href="/#projects"]');
  ...
  test.describe('Performance and Layout Stability', () => {
    test('No layout shift on page load', async ({ page }) => {
      for (const testPage of allPages.slice(0, 5)) { // Test first 5 pages
        await page.goto(testPage.url);
